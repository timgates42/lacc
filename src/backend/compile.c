    ...
    int *sse_regs)
{
    ...
    *sse_regs = 0;
    ...
    for (i = 0; i < array_len(&st->operands); ++i) {
        ...
        if ((str_haschr(str, 'r') && !str_haschr(str, 'm'))
        ...
        } else if (!str_haschr(str, 'r') && str_haschr(str, 'm')) {
            sym->memory = 1;
        }
    }
}
...
static void compile__asm(struct asm_statement st)
{
    ...
    for (i = 0; i < array_len(&st.operands); ++i) {
        op = array_get(&st.operands, i);
        if ((str_haschr(op.constraint, '=') || str_haschr(op.constraint, '+'))
            && explicit_reg_constraint(op.constraint, &r))
        {
            store(r, op.variable);
        }
